-- ----------------------------------------------------------------------------
-- 15 vendas de teste pro membro TESTE, datadas do mês passado (fechado) —
...
declare
  ...
  i integer;
begin
  select id into v_member_id from members where coupon_code = 'TESTE';
  ...
  for i in 1..15 loop
    ...
    values (v_member_id, 'TEST-' || gen_random_uuid()::text, 'TESTE', 199.90, v_last_month + (i || ' days')::interval)
